...
class SemanticAnalyzer:
            ...
            if isinstance(stmt, ast.FunctionDecl):
                if stmt.name in self.functions:
                    ...
                    self.functions[stmt.name] = FunctionInfo(
                        return_type=stmt.return_type, params=stmt.params
                    )
    ...
    def _declare(
        self, name: str, info: SymbolInfo, line: int = 1, col: int = 1
    ) -> None:
        scope = self.scopes[-1]
        if name in scope:
            self._error(
                line, col, f"Redeclaration of variable '{name}' in the same scope"
            )
            return
        scope[name] = info
    ...
    def _visit_stmt(self, stmt: ast.Statement) -> None:
        ...
        if isinstance(stmt, ast.VarDeclaration):
            ...
            self._declare(
                stmt.name, SymbolInfo(type=stmt.type, declared_line=1, initialized=init)
            )
            if stmt.value is not None:
                ...
                if rhs_t is not None and not self._is_assignable(stmt.type, rhs_t):
                    self._error(
                        1,
                        1,
                        f"Type mismatch: cannot assign {rhs_t} to {stmt.type} variable '{stmt.name}'",
                    )
            return
            ...
            if sym is not None:
                if rhs_t is not None and not self._is_assignable(sym.type, rhs_t):
                    self._error(
                        1,
                        1,
                        f"Type mismatch: cannot assign {rhs_t} to {sym.type} variable '{stmt.name}'",
                    )
        ...
        if isinstance(stmt, ast.FunctionDecl):
            ...
            for ptype, pname in stmt.params:
                self._declare(
                    pname, SymbolInfo(type=ptype, declared_line=1, initialized=True)
                )
        ...
        if isinstance(stmt, ast.ReturnStatement):
            ...
            if stmt.value is None:
                self._error(
                    1, 1, f"Non-void function must return a value of type {expected}"
                )
                return
            actual = self._infer_expr_type(stmt.value)
            if actual is not None and not self._is_assignable(expected, actual):
                self._error(
                    1, 1, f"Return type mismatch: expected {expected}, got {actual}"
                )
            return
    ...
    def _infer_expr_type(self, expr: ast.Expression) -> Optional[HingType]:
        ...
        if isinstance(expr, ast.StringLiteral):
            # Treat string literals as the `dasha` type for compatibility
            # with examples and client expectations where `dasha` is used
            # for string values.
            return "dasha"
        ...
        if isinstance(expr, ast.BinaryOp):
            ...
            if (
                op == "/"
                and isinstance(expr.right, ast.FloatLiteral)
    ...
    def _numeric_result_type(
        self, lt: Optional[HingType], rt: Optional[HingType], op: str
    ) -> Optional[HingType]:
        if lt is None or rt is None:
            return None
        if lt not in ("poora", "dasha") or rt not in ("poora", "dasha"):
            self._error(
                1, 1, f"Operator '{op}' requires numeric operands, got {lt} and {rt}"
            )
            return None
    ...
    def _mark_initialized_from_scan_target(self, expr: ast.Expression) -> None:
        ...
        if (
            ...
            and expr.op == "&"
            and isinstance(expr.operand, ast.Identifier)
        ):
            ...
            return
        self._error(
            1, 1, "Invalid input target in lo(...): expected variable or &variable"
        )
